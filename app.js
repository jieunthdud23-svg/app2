...
function authError(code = "") {
  if (code.includes("operation-not-allowed")) return "Firebase Authentication에서 이메일/비밀번호 또는 Google 로그인을 사용 설정해 주세요.";
  if (code.includes("unauthorized-domain")) return "Firebase Authentication의 승인된 도메인에 현재 Vercel 주소를 추가해 주세요.";
  if (code.includes("permission-denied")) return "Realtime Database 규칙이 게시되지 않았거나 현재 사용자에게 저장 권한이 없습니다.";
  if (code.includes("network-request-failed")) return "네트워크 연결을 확인한 뒤 다시 시도해 주세요.";
  if (code.includes("invalid-api-key") || code.includes("api-key-not-valid")) return "firebase-config.js의 API 키가 올바르지 않습니다.";
...
function goPage(page) {
  ...
  $(`#page-${page}`)?.scrollTo({ top: 0, behavior: "smooth" });
}
